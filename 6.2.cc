...
int main(){
	...
	for(int i=0;i<5;i++){ //Para el input tienes que utilizar 486 en la i, recuerdalo Alberto
		for(j;j<16;j++){ //2199 para la j
			if(test[j] != " "){
				inputProcessed[i] += test[j];
				saltos++;
			}else{
				lengthForTest[i] = saltos;
				saltos=0;
				i++;
			}
		}
	...
	for(int i=0;i<5;i++){ //Para mostrar el input pon 486 Alberto
		cout << i << " " << lengthForTest[i] << " " << inputProcessed[i] << endl;		
	}
	...
	for(int a=0;a<5;a++){
		...
		length = inputProcessed[a].length(); 
		inputProcessedSeparated = inputProcessed[a];
