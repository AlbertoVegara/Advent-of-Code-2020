...
int main(){
	...
	string inputProcessed[486]; //String con todos los grupos ya procesados
	
	int j =0;
	
	for(int i=0;i<486;i++){ //Para el input tienes que utilizar 486 en la i, recuerdalo Alberto
		for(j;j<2199;j++){
			if(input[j] != " "){
				inputProcessed[i] += input[j];
			}else{
				i++;
			}
		}
	...
	//Muestra todos los grupos ya procesados
	
	cout << "Muestro todo lo que contiene el array inputProcessed" << endl;
	
	for(int i=0;i<486;i++){ //Para mostrar el input pon 486 Alberto
		cout << i << " " << inputProcessed[i] << endl;		
	}
	
	//Aquí empieza la solución del ejercicio
	
	cout << endl;
	cout << "inputProcessed after eliminate duplicates" << endl;
	
	int length; //Variable para length del inputProcessed[a]
	string inputProcessedSeparated; //Variable que almacena uno por uno los inputProocessed[a] para poder eliminar los duplicados de cada uno
	int answer=0;
	
	for(int a=0;a<486;a++){ //Para movernos por cada inputProcessed[a]
		length = inputProcessed[a].length(); //Length del inputProcessed
		inputProcessedSeparated = inputProcessed[a]; //Cuando a=0 almacena abc, cuando es a=1 abc, cuando es a=2 abac, cuando es a=3 aaaa y cuando es a=4 almacena b
		//string inputProcessedSeparated[] = {'a', 'b', 'a', 'c'}
		
		for(int i=0;i<length;i++){
			for(int j=i+1;j<length;j++){
				if(inputProcessedSeparated[i] == inputProcessedSeparated[j]){
					for(int k=j;k<length;k++){
						inputProcessedSeparated[k] = inputProcessedSeparated[k+1];
					}
					length--;
					j--;
				}
		...
		inputProcessed[a] = inputProcessedSeparated;
		
	}
	
	cout << "Muestro todos los input eliminando duplicados" << endl;
	
	for(int i=0;i<486;i++){
		cout << inputProcessed[i] << endl;	
	}
	...
	for(int i=0;i<486;i++){
		//cout << inputProcessed[i] << endl;
		length = inputProcessed[i].length(); 
		inputProcessedSeparated = inputProcessed[i];
		for(int j=0;j<length;j++){
			if(inputProcessedSeparated[j] == 'a' || inputProcessedSeparated[j] == 'b' || inputProcessedSeparated[j] == 'c' || inputProcessedSeparated[j] == 'd' || inputProcessedSeparated[j] == 'e' || inputProcessedSeparated[j] == 'f' || inputProcessedSeparated[j] == 'g' || inputProcessedSeparated[j] == 'h' || inputProcessedSeparated[j] == 'i' || inputProcessedSeparated[j] == 'j' || inputProcessedSeparated[j] == 'k' || inputProcessedSeparated[j] == 'l' || inputProcessedSeparated[j] == 'm' || inputProcessedSeparated[j] == 'n' || inputProcessedSeparated[j] == 'o' || inputProcessedSeparated[j] == 'p' || inputProcessedSeparated[j] == 'q' || inputProcessedSeparated[j] == 'r' || inputProcessedSeparated[j] == 's' || inputProcessedSeparated[j] == 't' || inputProcessedSeparated[j] == 'u' || inputProcessedSeparated[j] == 'v' || inputProcessedSeparated[j] == 'w' || inputProcessedSeparated[j] == 'x' || inputProcessedSeparated[j] == 'y' || inputProcessedSeparated[j] == 'z'){
				answer++;
			}
		}
